...
- **Modo de pago dominical (por hora vs por día):** replica el patrón salario/tarifa → defaults en empresa (`default_dominical_payment_mode` `hour|day`, `default_normal_day_value` COP = valor del día normal) en `surcharge_rules` que **siembran** al crear el empleado; y valor propio por empleado (`dominical_payment_mode`, `normal_day_value` en `employees`) que es el que usan los cálculos. `hour` = comportamiento actual (`horas × tarifa × recargo`); `day` = la base por horas se paga siempre (como ordinario/nocturno) y encima el recargo por cada dominical pagado = `valor_día_normal × recargo% (sunday_holiday, 75% por defecto)`, sin importar las horas.
...
- **Backend:**
  - Migración 1: agregar `dominical_weekday`, `pay_dominical_by_default`, `default_dominical_payment_mode`, `default_normal_day_value` a `surcharge_rules`.
  - Migración 2: agregar `dominical_payment_mode`, `normal_day_value` a `employees`.
