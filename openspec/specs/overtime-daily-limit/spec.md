...
`CalculateWorkHours` SHALL clasificar como `overtime_hours` cualquier minuto neto que supere `max_daily_hours` en el día calendario actual, incluso si el acumulado semanal no ha alcanzado `max_weekly_hours`.

#### Scenario: Turno largo en día aislado dispara overtime diario
- **WHEN** `max_daily_hours = 8` y `max_weekly_hours = 42`
- **WHEN** un empleado trabaja 10h netas en un solo día sin horas previas esa semana
- **THEN** `regular_hours = 8.0` y `overtime_hours = 2.0`

#### Scenario: Límite semanal sigue activo como trigger independiente
- **WHEN** `max_daily_hours = 8` y `max_weekly_hours = 42`
- **WHEN** un empleado trabaja 7h por día durante 6 días (42h) y trabaja 1h el séptimo día
- **THEN** la hora del séptimo día es `overtime_hours = 1.0` (semanal agotado)
...
- **WHEN** trabaja un turno de 4h en ese día
- **THEN** las primeras 2h del turno son ordinarias y las últimas 2h son overtime
