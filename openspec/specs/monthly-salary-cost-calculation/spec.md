...
`CalculateReportCosts` SHALL ramificar el cálculo según el `salary_type` del empleado. En modo `monthly`, el costo SHALL componerse del salario base del periodo más el auxilio de transporte del periodo (cuando el empleado lo recibe), más solo los porcentajes de recargo y el valor completo de las horas extra; en modo `hourly`, el comportamiento SHALL ser idéntico al actual.
...
- El **auxilio de transporte** del periodo (prorrateado, ver capability `transport-allowance`) SHALL sumarse al `total` como concepto propio cuando el empleado lo recibe. NO es base de recargos ni de horas extra y NUNCA se multiplica por horas.
- El `total` SHALL incluir el salario base del periodo (prorrateado, ver capability `payroll-pay-period`), el auxilio de transporte del periodo, los recargos y las horas extra.
- `valor_hora` es el `hourly_rate` del empleado.

**Business Rules (modo `hourly`):**
- El cálculo SHALL ser idéntico al actual: cada bucket suma `horas × valor_hora × (1 + % / 100)`, con `regular` al 0% de recargo. No se suma salario base ni auxilio de transporte.

**Composición con `payOvertime`:**
- El flag `payOvertime` existente SHALL seguir aplicando: cuando es `false`, las 4 categorías de hora extra se calculan en `0`, se excluyen del `total` y se marcan `compensated: true`, en ambos modos de salario. El salario base, el auxilio de transporte y los recargos no se ven afectados por el flag.
...
- **AND** el `total` es el salario base del periodo más el auxilio de transporte (si lo recibe)

#### Scenario: El auxilio de transporte suma al total en modo mensual
- **WHEN** se calcula el costo de un empleado `monthly` que recibe auxilio para una quincena completa
- **THEN** el `total` incluye `transport_allowance / 2` además del salario base, recargos y extras
- **AND** el auxilio aparece como concepto propio separado del salario base
...
- **AND** no se suma ningún salario base ni auxilio de transporte
...
- **THEN** el `total` de ambas quincenas es el mismo salario base del periodo más el mismo auxilio del periodo
...
- **AND** el salario base, el auxilio de transporte y los recargos del periodo sí suman al `total`
