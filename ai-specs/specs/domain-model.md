...
- Actions (reportes): GenerateCompanyReport, GenerateEmployeeReport (daily_breakdown enriquecido: horario ISO 8601, status con 'in_progress' derivado para turnos abiertos —no suman a totals— y breaks[] anidadas; cuentan días dominicales trabajados N; en modo `weekly` las horas extra se suman por la ventana de liquidación y exponen `overtime_settlement` + flag `overtime_deferred` por día), CalculateWeeklyOvertimeSettlement (calcula en minutos el saldo firmado de cada semana completa, compensa déficits y excedentes dentro de la ventana y devuelve overtime trabajado/liquidable + déficit informativo sin persistir negativos), CalculateReportCosts (acepta flag payOvertime + config dominical: pay/mode/day_value/payable_count/worked_days; modo `day` = base por horas + plus por día pagado (valor_día_normal × sunday_holiday%); festivo siempre paga, configurable por hora/día sin conteo editable), ResolveOvertimePaymentDecision y ResolveDominicalPaymentDecision (precedencia request → decisión guardada → default), ResolveOvertimeSettlementWindow (regla "dueño del domingo": en modo `weekly` el recargo extra de cada semana ISO se liquida en el periodo que contiene su domingo; devuelve la ventana de fechas del overtime)
